...
def update_graph(mode_slctd, station_slctd): # each argument connects to an input (above). One input = one argument, two inputs = two arguments etc. The argument always refers to the component property
    ...
    bar_chart_df = bar_chart_df[(bar_chart_df['subsystem'] == mode_slctd) & (bar_chart_df['startstn'] == station_slctd)]
   ...
   # Calculate the number of journeys from each start station
    scatterplot_df = df.copy()
    scatterplot_df = scatterplot_df[(scatterplot_df['subsystem'] == mode_slctd) ]
    ...
    # Merge with the original dataframe to get longitude and latitude
